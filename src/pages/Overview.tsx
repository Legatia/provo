  ...
  return (
    <Card className="relative overflow-hidden p-4">
      ...
      <div className="flex items-center justify-between">
        <span className="text-[10px] font-semibold uppercase tracking-[0.16em] text-paper-dim">
          {label}
        </span>
      ...
      {sub && <div className="mt-1.5 text-[11px] leading-none text-paper-dim">{sub}</div>}
    </Card>
  );
}
  ...
  if (overview === null) {
    return (
      <Card className="mx-auto max-w-lg">
        <EmptyState
          icon="◈"
          title="The desk isn't provisioned yet"
          hint="Run setup from the Demo panel to provision the monitored entity, watch rules and sources."
        />
        <div className="flex justify-center pb-6">
          <Link
            to="/demo"
            className="rounded-xl bg-brass/90 px-4 py-2 text-sm font-medium text-white shadow-[0_0_20px_rgba(201,162,75,0.25)] transition hover:bg-brass"
          >
            Open Demo Panel →
          </Link>
        </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-baseline justify-between gap-2">
        <div>
          <h2 className="text-lg font-semibold tracking-tight">
            {overview.company.name} · <span className="text-paper-dim">{overview.company.product}</span>
          </h2>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        <StatCard label="Emerging" value={overview.counts.emerging} icon="⚠️" accent="#D9A62E" loading={false} />
      ...
      <div className="grid gap-5 lg:grid-cols-5">
        ...
        <div className="space-y-5 lg:col-span-3">
          ...
          <div>
            <SectionTitle
              right={
                <span className="font-mono text-[10px] text-paper-dim/70">last 14 days</span>
              }
            >
              Signal volume
            </SectionTitle>
            <Card className="p-4">
              <BarChart
                ...
                color="#C9A24B"
              />
              <div className="mt-2 flex items-center justify-between text-[10px] text-paper-dim/70">
              ...
              <div className="mt-3 border-t border-rule pt-3">
                <MixBar
                  parts={[
                    { label: "email", count: overview.emailSignals, color: "#58B98C" },
                    { label: "reddit", count: overview.webSignals, color: "#C9A24B" },
                  ]}
                />
              </div>
            </Card>
          ...
          <div>
            ...
            <Card hover className="divide-y divide-white/[0.05]">
              ...
              {overview.recentChanges.map((c: any) => (
                <Link key={c._id} to={`/issues/${c._id}`} className="group flex items-center gap-4 p-4 transition hover:bg-paper/[0.015]">
                  <div className="min-w-0 flex-1">
                    <div className="truncate text-[13px] font-medium text-paper group-hover:text-white">
                      {c.title}
                    </div>
                  ...
                  <Sparkline
                    ...
                    width={90}
                    height={26}
                    stroke={(c.growthMultiplier ?? 1) > 1 ? "#f87171" : "#58B98C"}
        ...
        <div className="lg:col-span-2">
          <SectionTitle right={<span className="flex items-center gap-1.5 text-[10px] text-verdigris"><span className="relative flex h-1.5 w-1.5"><span className="absolute h-full w-full animate-ping rounded-full bg-verdigris opacity-60" /><span className="relative h-1.5 w-1.5 rounded-full bg-verdigris" /></span>live</span>}>
            Agent activity
          </SectionTitle>
          <Card className="max-h-[560px] overflow-y-auto p-2">
            ...
            <div className="relative">
              {overview.activity.map((t: any, i: number) => (
                <div key={t._id} className="animate-fade-up relative flex gap-3 rounded-xl p-2.5 transition hover:bg-paper/[0.015]">
                  <TypeIcon type={t.type} running={t.status === "running"} />
                  <div className="min-w-0 flex-1">
                    <div className="flex items-start justify-between gap-2">
                      <span
                        className={`text-[12.5px] leading-snug ${
                          t.status === "running" ? "text-brass-bright" : "text-paper/90"
                        }`}
                      >
                        {t.label}
                      </span>
                    ...
                    {t.detail && (
                      <p className="mt-0.5 line-clamp-2 text-[11px] leading-relaxed text-paper-dim">
                        {t.detail}
                      </p>
                    )}
                  </div>
